 ...
 */
@Slf4j
public class HttpResponseConvert extends ResponseConverter<Response> {
    ...
    @Override
    public InvokeResponse convert(Response response) throws Exception {
        ...
        if (response != null) {
            ...
            if (response.isSuccessful()) {
                ...
                if (response.body() != null) {
                    ...
                    String contentType = headers.get(HttpConstant.CONTENT_TYPE);
                    log.debug("请求返回的文件内容类型为：{}",contentType);
                    if (StringUtils.contains(contentType, HttpConstant.JSON)) {
                ...
                throw new KipaProcessException("http调用失败，错误信息：" + message + "。服务端详细返回信息：\n" +error);
            }
        }
        return httpResponse;
    }
